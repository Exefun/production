...
export default function Support({ setActiveTab }: SupportProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (Object.keys(newFieldErrors).length > 0) {
      console.warn(
        "🟨 SUPPORT: Frontend validation failed",
        newFieldErrors
      );
    ...
    try {
       ...
       * This is optional.
       *
       * Anonymous visitors can still submit support
       * tickets exactly as before.
       *
       * Logged-in users have their ticket associated
       * with their dashboard account.
       */
      ...
      console.log(
        "🟩 SUPPORT: Parsed server response:",
        data
      );
      ...
      console.log(
        "🟦 SUPPORT: Submission process finished"
      );
    }
  };
