...
interface AuthProps {
  onLogin: (user: { name: string; email: string; avatarColor: string; avatarTag: string; focusArea: string }) => void;
  setActiveTab: (tab: ActiveTab, mode?: 'login' | 'register') => void;
  initialMode?: 'login' | 'register';
}

export default function Auth({ onLogin, setActiveTab, initialMode = 'login' }: AuthProps) {
  ...
  const [error, setError] = useState<string | null>(
    'The Exefun Cognitive Trial registration and login portals are currently suspended. The active research cohort is fully booked, and we are not accepting new entries or logins at this time. Please check back during the next public trial phase.'
  );
  ...
  const validateEmail = (emailStr: string) => {
    // Robust RFC 5322 matching regex
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setError('The Exefun Cognitive Trial registration and login portals are currently suspended. The active research cohort is fully booked, and we are not accepting new entries or logins at this time. Please check back during the next public trial phase.');
  };
  ...
  return (
    <div className="w-full min-h-[85vh] flex items-center justify-center px-4 py-16 sm:py-24 bg-slate-50 relative overflow-hidden" id="auth-root">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="flex items-center justify-between mb-6">
          <nav className="flex items-center space-x-2 text-[10px] sm:text-xs font-mono tracking-wider text-slate-600 bg-white border border-slate-200 rounded-full py-1.5 px-4 shadow-sm">
            ...
            <span className="text-indigo-600 font-extrabold">
              {isRegister ? 'REGISTRATION' : 'PORTAL SIGN IN'}
            </span>
          </nav>
          ...
          {isSuccess && (
            ...
            >
              ...
              <p className="text-slate-500 text-xs sm:text-sm font-sans mt-2 max-w-xs leading-relaxed">
                {isRegister 
                  ? 'Your academic research account has been generated! Initializing cognitive dashboard.' 
                  : 'Welcome back! Synaptic baseline logs synchronized successfully.'}
              </p>
              <div className="flex items-center space-x-2 mt-6 text-indigo-600 text-xs font-mono">
                <Sparkles className="h-4 w-4 animate-spin" />
                <span>REDIRECTING TO EXEFUN HOME...</span>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="bg-white border border-slate-100 rounded-3xl p-6 sm:p-10 shadow-2xl relative overflow-hidden">
          ...
          <div className="text-center space-y-3 mb-8">
            ...
            <div>
              <h2 className="text-xl sm:text-2xl font-sans font-extrabold text-slate-900 tracking-tight">
                {isRegister ? 'Create Your Account' : 'Portal Sign In'}
              </h2>
          ...
          <div className="space-y-4">
            ...
            >
              {/* Google Brand SVG */}
              <svg className="h-5 w-5 group-hover:scale-105 transition-transform" viewBox="0 0 24 24" fill="none">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-left">
              ...
              {isRegister && (
                <div className="grid grid-cols-1 gap-4">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className={`w-full bg-slate-50 border ${
                          fieldErrors.fullName ? 'border-red-500 bg-red-50/20 focus:border-red-500' : 'border-slate-200 focus:border-blue-500'
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-mono font-bold tracking-wider text-slate-500 uppercase">
                      Contact Number <span className="text-slate-400 font-medium">(10 digits)</span>
                    </label>
                    <div className="relative">
                      <div className="absolute inset-y-0 left-3 flex items-center pointer-events-none text-slate-400">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (fieldErrors.contactNumber) setFieldErrors(prev => ({ ...prev, contactNumber: false }));
                        }}
                        placeholder="9876543210"
                        className={`w-full bg-slate-50 border ${
                          fieldErrors.contactNumber ? 'border-red-500 bg-red-50/20 focus:border-red-500' : 'border-slate-200 focus:border-blue-500'
                        } rounded-xl py-2.5 pl-10 pr-4 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:bg-white transition-all font-sans`}
                      />
                    </div>
                  </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full bg-slate-50 border ${
                      fieldErrors.email ? 'border-red-500 bg-red-50/20 focus:border-red-500' : 'border-slate-200 focus:border-blue-500'
                    } rounded-xl py-2.5 pl-10 pr-4 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:bg-white transition-all font-sans`}
                  />
                </div>
              </div>
              ...
              {/* Password Fields */}
              <div className={isRegister ? "grid grid-cols-1 sm:grid-cols-2 gap-4" : "space-y-1.5"}>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="••••••••"
                      className={`w-full bg-slate-50 border ${
                        fieldErrors.password ? 'border-red-500 bg-red-50/20 focus:border-red-500' : 'border-slate-200 focus:border-blue-500'
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="••••••••"
                        className={`w-full bg-slate-50 border ${
                          fieldErrors.confirmPassword ? 'border-red-500 bg-red-50/20 focus:border-red-500' : 'border-slate-200 focus:border-blue-500'
                        } rounded-xl py-2.5 pl-10 pr-10 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:bg-white transition-all font-sans`}
                      />
                      <button
                        type="button"
              ...
              >
                {isLoading ? (
                  ...
                  <>
                    <span>{isRegister ? 'Generate Your Account' : 'Authenticate Portal Access'}</span>
